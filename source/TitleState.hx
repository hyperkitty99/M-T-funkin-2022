	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			if(FlxG.sound.music == null) {
				FlxG.sound.playMusic(Paths.music('mt_funkin_menu_fnf_real'), 0);
			}
		}

		Conductor.changeBPM(90.187);
		persistentUpdate = true;

		var bg:FlxSprite = new FlxSprite();

		bg.loadGraphic(Paths.image('titlestuff/bg'));
		...
		logoBl.frames = Paths.getSparrowAtlas('titlestuff/logo_bumpin');
		...
		var bars:FlxSprite = new FlxSprite(0, -3);
		bars.loadGraphic(Paths.image('titlestuff/barStuff'));
		bars.setGraphicSize(Std.int(1285));
		bars.updateHitbox();
		add(bars);
