...
class Alphabet extends FlxSpriteGroup
{
	...
	public var isMenuSpin:Bool = false;
	...
	public var isrole:Bool = false;
	public var ismistik:Bool = false;
	public var isjustjack:Bool = false;
	...
	override function update(elapsed:Float)
	{
		...
		if (isMenuItemX) {
			...
			var lerpVal:Float = CoolUtil.boundTo(elapsed * 9.6, 0, 1);
			x = FlxMath.lerp(x, (scaledY * yMult) + (FlxG.height * 0.48) + yAdd, lerpVal);
			if(forceX != Math.NEGATIVE_INFINITY) {
				x = forceX;
			} else {
				x = FlxMath.lerp(x, (targetY * 20) + 90 + xAdd, lerpVal);
		...
		if (isMenuSpin) {
			...
			x = FlxMath.lerp(x, Math.exp(Math.abs(scaledY * 0.6)) * 70 + (FlxG.width * 0.01), 0.16 / (openfl.Lib.application.window.frameRate / 60));
		}
