...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if(FlxG.save.data.beatenMid) { //Even i'm admitting this lmao 
			WeekData.weeksList.push("week3");
			WeekData.weeksLoaded.set("week3", new WeekData( {
				songs: [["372", "372", [255, 113, 253]]],
				weekCharacters: ['', '', ''],
				weekBackground: '',
				weekBefore: 'week1',
				storyName: '372',
				weekName: '372',
				freeplayColor: [255, 113, 253],
				startUnlocked: true,
		...
		dots.scale.set(1.2, 1.2);
		dots.screenCenter();
		dots.antialiasing = ClientPrefs.globalAntialiasing;
		add(dots);

		var vingette:FlxSprite = new FlxSprite(0, 0).loadGraphic(Paths.image('vingette'));
		vingette.antialiasing = ClientPrefs.globalAntialiasing;
		vingette.setGraphicSize(Std.int(1300));
        ...
        mt = new FlxSprite(-300, -200).loadGraphic(Paths.image('mistik'));
        mt.antialiasing = ClientPrefs.globalAntialiasing;
		mt.setGraphicSize(Std.int(1300));
        add(mt);
		...
		CoolUtil.defaultDifficulty = 'Hard';
		curDifficulty = 2;
		
		changeSelection();
		changeDiff();

		var text:FlxText = new FlxText(FlxG.height, FlxG.width, '', 16);
	...
	override function update(elapsed:Float)
	{
		...
		if(songs.length > 1)
		{
			...
			//if(showchar != null && showchar.animation.curAnim.finished) {
				//showchar.dance();
			//}
